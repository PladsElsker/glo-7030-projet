...
sys.path.append(str(Path(__file__).parent / "uni_sign"))
...
def main() -> None:
    ...
    state_dict = torch.load(config.pretrained_weights["Stage1"], weights_only=True)
    model.load_state_dict(state_dict["model"], strict=False)
    model.to("cuda")
    x = preprocess_data(
        Path("uni_sign/data/CSL_News/rgb_format/Common-Concerns_20210925_44612-44912_16547.pkl"),
